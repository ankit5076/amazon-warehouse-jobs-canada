...
- If WAF/CAPTCHA logs show `page bridge script load failed` for a hashed `direct-waf-bridge-page.<hash>.js`, compare that hash with the current built `dist/amazon-shifts/manifest.json`. A stale content script after extension reload can keep referencing a deleted old bridge bundle; the operator must reload the extension and refresh/close Amazon tabs so content and web-accessible bundle hashes match.
